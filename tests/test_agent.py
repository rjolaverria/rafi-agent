...
class TestAgentRun:
    @pytest.mark.asyncio
    async def test_returns_on_no_tool_calls(self, model: Model, mock_client: MagicMock):
    ...
    @pytest.mark.asyncio
    async def test_raises_on_empty_choices(self, model: Model, mock_client: MagicMock):
    ...
    @pytest.mark.asyncio
    async def test_system_prompt_prepended(self, model: Model, mock_client: MagicMock):
        mock_client.chat.completions.create.return_value = _make_completion()
    ...
    @pytest.mark.asyncio
    async def test_no_system_prompt_not_prepended(
    ...
    @pytest.mark.asyncio
    async def test_executes_tool_and_loops(self, model: Model, mock_client: MagicMock):
        ...
        agent = Agent(model, agent_tools=[FakeTool])
        await agent.run([{"role": "user", "content": "do it"}])
    ...
    @pytest.mark.asyncio
    async def test_assistant_message_appended_with_tool_calls(
        self, model: Model, mock_client: MagicMock
    ):
    ...
    @pytest.mark.asyncio
    async def test_tools_schema_passed_to_api(
        self, model: Model, mock_client: MagicMock
    ):
        mock_client.chat.completions.create.return_value = _make_completion()
        agent = Agent(model, agent_tools=[FakeTool])
    ...
    @pytest.mark.asyncio
    async def test_model_name_passed_to_api(self, model: Model, mock_client: MagicMock):
        mock_client.chat.completions.create.return_value = _make_completion()
        agent = Agent(model, agent_tools=[])
        await agent.run([{"role": "user", "content": "hi"}])
        call_args = mock_client.chat.completions.create.call_args
    ...
    @pytest.mark.asyncio
    async def test_iterations_reset_between_runs(
        self, model: Model, mock_client: MagicMock
    ):
        ...
        agent = Agent(model, agent_tools=[FakeTool])
        await agent.run([{"role": "user", "content": "first"}])
        assert agent.state.iterations == 2

        mock_client.chat.completions.create.side_effect = [
            _make_completion(content="done"),
        ]
        await agent.run([{"role": "user", "content": "second"}])
...
class TestExecuteTool:
    @pytest.mark.asyncio
    async def test_executes_known_tool(self, model: Model):
        agent = Agent(model, agent_tools=[FakeTool])
        result = await agent._execute_tool("faketool", json.dumps({"value": "hi"}))
        assert result == "got:hi"

    @pytest.mark.asyncio
    async def test_unknown_tool_returns_error(self, model: Model):
        agent = Agent(model, agent_tools=[FakeTool])
        result = await agent._execute_tool("nope", "{}")
        assert "Unknown tool" in result
        assert "nope" in result

    @pytest.mark.asyncio
    async def test_invalid_json_returns_error(self, model: Model):
        agent = Agent(model, agent_tools=[FakeTool])
        result = await agent._execute_tool("faketool", "not json")
        assert "not valid JSON" in result

    @pytest.mark.asyncio
    async def test_tool_execution_error_returns_error(self, model: Model):
        agent = Agent(model, agent_tools=[FailingTool])
        result = await agent._execute_tool("failingtool", "{}")
        assert "error" in result.lower()
        assert "tool blew up" in result

    @pytest.mark.asyncio
    async def test_triggers_tool_call_hook(self, model: Model):
        hook = MagicMock()
        agent = Agent(model, agent_tools=[FakeTool], on_tool_call=hook)
        await agent._execute_tool("faketool", json.dumps({"value": "x"}))
        hook.assert_called_once_with("faketool", {"value": "x"})

    @pytest.mark.asyncio
    async def test_triggers_tool_result_hook(self, model: Model):
        hook = MagicMock()
    ...
    @pytest.mark.asyncio
    async def test_hooks_not_called_for_unknown_tool(self, model: Model):
    ...
    @pytest.mark.asyncio
    async def test_uses_default_field_value(self, model: Model):
        agent = Agent(model, agent_tools=[FakeTool])
        result = await agent._execute_tool("faketool", "{}")
...
class TestAgentRunHooks:
    @pytest.mark.asyncio
    async def test_response_hook_called(self, model: Model, mock_client: MagicMock):
        mock_client.chat.completions.create.return_value = _make_completion(
            content="hi"
        )
        hook = MagicMock()
    ...
    @pytest.mark.asyncio
    async def test_response_hook_called_each_iteration(
        self, model: Model, mock_client: MagicMock
    ):
        tc = _make_fn_tool_call("faketool", {"value": "a"})
        mock_client.chat.completions.create.side_effect = [
